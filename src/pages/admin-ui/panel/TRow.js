...
function TBody({ data, item, setData, setTotalPages }) {
     ...
     * Updates the background color of the row based on checkbox selection.
     * @param {Object} e - The event object.
     */
    const checkboxChangeHandler = (e) => {
        if (e.target.checked) {
            e.target.parentElement.parentElement.style.background = '#e0e0e0';
        } else {
    ...
    return (
        <tr key={item.id} className='admin-table-row hover:bg-gray-200'>
            <td className='admin-table-column tRow-checkbox'>
                <input id={item.id} type='checkbox' onChange={(e) => checkboxChangeHandler(e)} />
            </td>
